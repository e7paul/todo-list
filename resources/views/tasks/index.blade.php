<x-base>
            ...
            <tr>
                <td>
                    <form method="POST" action="{{ route('tasks.update', $task) }}">
                        @csrf
                        @method('patch')
                        <input type="checkbox" name="is_done" value="1" onclick="event.preventDefault(); this.closest('form').submit();" @checked($task->is_done)
    ...
    <div class="mx-auto mt-5 w-50">
        <form method="POST" action="{{ route('tasks.store') }}">
            @csrf
            <textarea name="message" placeholder="{{ __('Describe your task') }}" class="block w-full border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm">{{ old('message') }}</textarea>
